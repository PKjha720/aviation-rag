...
Whether this reflects tabular content itself or the length of table-derived chunks is not separable in this design; see Limitations. The within-slice split by length points to the latter.
...
|---|---|---|
| Tables flattened into prose by `get_text("text")` | 368 table-derived chunks after the fix | **Fixed** — extracted via `find_tables()`, emitted as markdown, flagged `is_table` |
...
**Table detection needs a validity filter.** `find_tables()` fires readily on numbered lists and indented prose. Of 786 initial detections, 57.4% had fewer than two columns populated in a majority of rows — typically one populated column flanked by empty ones, as an enumerated list produces. Requiring ≥2 columns populated in >50% of rows and ≥3 rows keeps a minority of the 786 candidates; after row-wise chunking these yield 368 table-derived chunks. Rejected regions fall back to the prose path rather than being discarded.
